...
const WardScheduleSystem = () => {
  ...
  const handlePrevMonthUpload = (e) => {
    ...
    reader.onload = (event) => {
      try {
        const data = new Uint8Array(event.target!.result as ArrayBuffer);
        const workbook = XLSX.read(data, { type: 'array' });
        // シート名は固定せず先頭シートを使う（CWSは「C1030036_共立4F西」等のコード付き）
        const sheetName = workbook.SheetNames[0];
        const worksheet = workbook.Sheets[sheetName];
        const jsonData = XLSX.utils.sheet_to_json(worksheet, { header: 1, defval: '' });

        // 前月末7日分のデータを抽出（CWS変換ファイルにも対応）
  ...
  const detectDateHeaderRun = (row) => {
    ...
    for (let j = 0; j < ints.length; j++) {
      if (ints[j] === 1) {
        ...
        // 15日以上の連続並びのみ日付ヘッダーと判定（職員番号等の誤検出防止）
  ...
  // 前月末7日分のデータを抽出（CWS変換ファイル対応）。
  // 戻り値: { rows:[{name,empNo,shifts,rowIndex}], warnings:[{name,day,symbol}], patternWarnings:[{name,day,symbol,detail}], totalStaff, error }
  const extractPreviousMonthDataAsArray = (jsonData) => {
    const empty = { rows: [], warnings: [], patternWarnings: [], totalStaff: 0, error: null as string | null };
    if (!jsonData || jsonData.length < 2) {
      return { ...empty, error: 'ファイルにデータがありません。' };
    }

    // 1. ヘッダー行の自動検出（1〜15行目を走査。位置は決め打ちしない）
    ...
    for (let i = headerRowIndex + 1; i < jsonData.length; i++) {
      ...
      // 氏名セルに文字列があること（下部の集計行「看護（深）」等は氏名列が空なので自然に除外）
      const name = String(row[nameCol] ?? '').trim();
      if (!name) continue;

      // 日付列範囲に1つ以上の値があること
      const hasAnyDate = dateCols.some((dc) => String(row[dc.col] ?? '').trim() !== '');
      ...
      for (let d = startIdx; d < dateCols.length; d++) {
        ...
        if (norm === '') {
          // マップにない記号：エラーにせず記録し、そのセルは空扱いで続行
          warnings.push({ name, day: dc.day, symbol: raw });
          shifts.push('');
        } else {
          shifts.push(norm);
        }
      ...
      // 夜勤パターンの整合性チェック（取り込む末尾7日分。末尾＝月末日は自動的に対象外）
      const winDays = dateCols.slice(startIdx).map((dc) => dc.day);
      checkNightPatternWarnings(name, shifts, patternWarnings, winDays);

      rows.push({ name, empNo, shifts, rowIndex: rows.length });
    }

    return { rows, warnings, patternWarnings, totalStaff, error: null };
  };
  ...
  // シフト記号を正規化
  // CWS記号は内部シフトへ統合: ・→日, 準→夜, 深→明, 管準→管夜, 管深→管明, 公→休, 年→有,
  //   短1/短4/遅1/遅4→日, ﾊ(半休)/ﾈ(年休)を含む複合記号→休
  const normalizeShift = (shift) => {
    if (!shift) return '';
    const s = String(shift).trim();
    if (s === '日' || s === '日勤' || s === 'D' || s === '・') return '日';
    if (s === '夜' || s === '夜勤' || s === 'N' || s === '準') return '夜';          // 準夜勤 → 夜
    if (s === '明' || s === '夜明' || s === '夜勤明' || s === 'A' || s === '深') return '明'; // 深夜勤 → 明
    // 管夜・管明はW4のSHIFT_TYPESに定義済みのため直接マッピング（フォールバック不要）
    if (s === '管夜' || s === '管理夜勤' || s === '管準') return '管夜';             // 管理準夜 → 管夜
    if (s === '管明' || s === '管理夜明' || s === '管理夜勤明' || s === '管深') return '管明'; // 管理深夜 → 管明
    if (s === '短1' || s === '短4' || s === '遅1' || s === '遅4') return '日';       // 時短・遅出は日勤扱い
    if (s === '休' || s === '公休' || s === '公' || s === 'O' || s === '0') return '休';
    if (s === '有' || s === '有休' || s === '有給' || s === 'Y' || s === '年') return '有';
    if (s === 'nan' || s === 'NaN') return '休';
    // 半休(ﾊ)・年休(ﾈ) を含む複合記号（例: '/ﾊ', 'ﾀ1/ﾈ' 等）は休扱い
    if (s.includes('ﾊ') || s.includes('ﾈ')) return '休';
    // システムシフトまたはカスタムシフトならそのまま返す（恒等変換）
    return VALID_SHIFTS.includes(s) || validShifts.includes(s) ? s : '';
  };

  // 変換後データの夜勤パターン整合性チェック（警告のみ。取り込みは止めない）。
  // ・「夜(管夜)」の翌日が「明(管明)」でない
  // ・「明(管明)」の翌日が「休」でも「夜(管夜)」でもない（夜明→休、夜明夜明→休休 が正常）
  ...
  const checkNightPatternWarnings = (name: string, shifts: string[], warnings: any[], days?: number[]) => {
    ...
    for (let i = 0; i < shifts.length - 1; i++) {
      ...
      if (!cur || !next) continue; // 空セルはスキップ（未対応記号は別途警告済み）
              ...
              <div className="bg-white rounded-2xl p-6 w-full max-w-6xl my-4">
                ...
                {prevMonthRawData.length > 0 ? (
                  <>
                    ...
                    {prevMonthImportReport && prevMonthImportReport.unmappedSymbols.length > 0 && (
                      <div className="mt-3 p-3 bg-orange-50 border border-orange-200 rounded-lg text-sm">
                        <p className="font-semibold text-orange-800 mb-1">
                          ⚠️ 未対応の勤務記号: {prevMonthImportReport.unmappedSymbols.length}件（該当セルは空として取り込みます）
                        </p>
                    ...
                    {prevMonthImportReport && prevMonthImportReport.patternWarnings && prevMonthImportReport.patternWarnings.length > 0 && (
